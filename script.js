...
function calculateNeravelleDate(realWorldDate) {
    ...
    // Calculate total days passed in Neravelle time
    const totalNeravelleDaysPassed = Math.floor(nvTimeDiff / (1000 * 60 * 60 * 24));
    
    // Calculate current year, month, day
    ...
    while (remainingDays > 0) {
        const daysInMonth = 35; // All months have 35 days
...
function updateSkyAnimation(nvHours, nvDay, nvDayName) {
   ...
   // Calculate sun/moon position (0-24 hours)
    ...
    sunMoon.style.animation = 'moon-glow 4s infinite';
        ...
        // Moon during night - determine phase
        const moonPhase = getMoonPhase(nvDay);
        sunMoon.className = `sun-moon moon-${moonPhase}`;
    ...
    if ((nvHours >= 17 && nvHours < 19) || (nvHours >= 5 && nvHours < 7)) {
        celestialSize = 150; // Larger at sunrise/sunset
    }
    ...
    sunMoon.style.opacity = celestialOpacity;
